...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from("agentquestionsoperationproperties")
      .insert(insertData)
      .select()
      .single()
    ...
    if (error) {
      ...
      return NextResponse.json(
        {
          error: "Failed to save agent questions operation",
...
export async function GET() {
  try {
    ...
    if (error) {
      console.error("Error fetching agent questions operations:", error)
      return NextResponse.json({ error: "Failed to fetch agent questions operations" }, { status: 500 })
    }

    return NextResponse.json({ data }, { status: 200 })
  } catch (error) {
    console.error("Error in agent questions operation API:", error)
    return NextResponse.json(
      {
        error: "Internal server error",
        details: error instanceof Error ? error.message : String(error),
